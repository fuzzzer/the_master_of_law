    ...
    return BlocConsumer<FeedbackCubit, FeedbackState>(
      ...
      builder: (context, state) {
        return Container(
          ...
          // 🔴 T-0257 (M14e). This sheet has **seven category chips, a
          // five-star rating row, a multi-line comment field and a submit
          // button**, and none of it was scrollable: the `Column` was the
          // Container's direct child, so at textScaler 1.3 / 360 dp it
          // overflowed the sheet by **123 px on the bottom** and the comment
          // field and the submit button were unreachable — the sheet could
          // not be completed at all.
          //
          // `isScrollControlled: true` (line 28) only lets the sheet grow past
          // half the screen; it does not make anything scroll. The framework's
          // own advice in that overflow message is the fix: "consider … using
          // a scrollable container rather than a Flex". `mainAxisSize.min` is
          // kept, so a short sheet still hugs its content and does NOT grow to
          // full height.
