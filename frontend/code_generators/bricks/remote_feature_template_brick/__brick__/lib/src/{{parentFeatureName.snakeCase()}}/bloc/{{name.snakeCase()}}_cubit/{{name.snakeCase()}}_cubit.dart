...
class {{name.pascalCase()}}Cubit extends Cubit<{{name.pascalCase()}}State> {
  ...
  Future<void> {{functionName.camelCase()}}({{#hasRequestData}}{
      required {{requestDataName.pascalCase()}} {{requestDataName.camelCase()}},
    }{{/hasRequestData}}) async {
    emit(state.copyWith(status: StateStatus.loading));

    final {{functionName.camelCase()}}Response = await {{name.camelCase()}}Repository.{{functionName.camelCase()}}({{#hasRequestData}}
