...
**Distribution:** 32 criminal, 25 civil, 3 administrative, 2 constitutional

---

## Step 2: Run AI Evaluation ← YOU ARE HERE
...
# Or override: export GCP_SA_KEY_PATH=/path/to/credentials.json
...
**Output:** `eval/test_cases/eval_results.json`
...
After Step 2 completes, the script prints a summary report:
...
| AI's actual response | `eval_results.json` → `results[].ai_response` |
...
├── run_eval.py                 ← Step 2: run AI evaluation
...
| Want to re-evaluate a case | Delete `eval_results.json` and re-run |
